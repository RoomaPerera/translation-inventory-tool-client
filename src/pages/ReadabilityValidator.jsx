...
import RoleBasedTabs from '../components/Tabs';
...
const ReadabilityValidator = () => {
  ...
  const handleTextAnalyze = async () => {
    ...
    let inputNameType = 'Plain Text';

    if (textToAnalyze.startsWith('{') && textToAnalyze.endsWith('}')) {
      inputNameType = 'JSON Text';
    } else if (textToAnalyze.includes(',') && textToAnalyze.includes('\n')) {
      inputNameType = 'CSV Text';
    }
                  ...
                  {savedResults.map((item, idx) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="border px-3 py-1 truncate">{item.inputName}</td>
