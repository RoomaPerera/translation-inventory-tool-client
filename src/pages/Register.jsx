...
const Register = () => {
  ...
  const selectedLanguageNames = languages
    .map((code) => allLangs?.find((lang) => lang.code === code)?.name)
    .filter(Boolean);
  ...
  const handleSubmit = async (e) => {
    ...
    if (result) {
      setSuccessMessage(
        "Registration submitted successfully. Your account is pending admin approval. Please wait for approval before attempting to log in."
      );
  ...
  return (
    <div className="flex min-h-screen w-full m-0 p-0 overflow-hidden">
      {/* Left Panel */}
      <div
        className="w-2/5 min-w-0 bg-gradient-to-b from-purple-800 to-teal-500 text-white flex flex-col 
      ...
      <div className="w-3/5 min-w-0 bg-gray-100 flex items-center justify-center min-h-screen py-8 overflow-hidden">
        <div
          className={`bg-white p-6 rounded shadow-md w-full max-w-sm transition-all duration-200 ${
            isLanguageDropdownOpen && role === "Translator"
              ? "h-full max-h-full"
              : "max-h-fit"
          }`}
        >
                      ...
                      <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg overflow-hidden">
                        ...
                        <div className="max-h-24 overflow-y-auto">
                          {filteredLanguages.length > 0 ? (
                            ...
                            <div className="p-2 text-gray-500 text-sm h-8 flex items-center">
                              No languages found
                            </div>
                          )}
                        </div>
