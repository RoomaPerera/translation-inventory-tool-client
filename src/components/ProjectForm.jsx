  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (project?._id) {
        ...
        result = await projectService.updateProject(project._id, projectData);
        setSuccess('Project updated successfully!');
      } else {
        // Create new project - include createdBy field
        if (user?._id) {
          projectData.createdBy = user._id;
        }
        console.log('Creating new project data:', projectData);
        result = await projectService.addProject(projectData);
      ...
      const action = project?._id ? 'update' : 'create';
      setError(err.message || `Failed to ${action} project. Check if the server is running.`);
    } finally {
