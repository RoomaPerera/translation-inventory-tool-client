...
const EditTranslationModal = ({ isOpen, onClose, onSave, translation, currentUser, userService }) => {
  ...
  const canEditMetadata = currentUser?.role === 'admin' || currentUser?.role === 'developer';
  const canEditTranslation = ['admin', 'developer', 'translator'].includes(currentUser?.role);
  ...
  useEffect(() => {
    if (debouncedKey) {
      const fetchNlpData = async () => {
        ...
        try {
          const suggestRes = await nlpService.getSuggestions(debouncedKey, formData.product);
      ...
      case 'translation-updated':
        if (data.userId !== currentUser?.id) {
          ...
          if (hasUnsavedChanges) {
            ...
            setFormData(prevData => ({ ...prevData, translatedText: data.translatedText }));
  ...
  const handleChange = (e) => {
    ...
    if ((name === 'translationKey' || name === 'language') && !canEditMetadata) {
    ...
    if (name === 'translatedText' && wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
  ...
  const handleConflictResolve = (resolution, localText = null) => {
    if (resolution === 'accept-server') {
      setFormData(prevData => ({ ...prevData, translatedText: conflictData.translatedText }));
      ...
      <div className="modal-content" style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            ...
            <div className="subtitle">{formData.product}: {translation?.translationKey} ({translation?.language?.toUpperCase()})</div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
              <div className="space-y-5">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1">
                    ...
                    {!canEditMetadata && <span className="text-xs text-gray-400 ml-2">(Read-only)</span>}
                  </label>
                  <input
                    type="text"
                    name="translationKey"
                ...
                <div className="relative">
                  <textarea
                    name="translatedText"
                    placeholder={canEditTranslation ? "Translated Text" : "You don't have permission to edit translations"}
                    rows="4"
              ...
              <div className="flex justify-between items-center mt-auto pt-6">
                ...
                <div className="flex gap-4">
                  ...
                  <button type="submit" disabled={isSaving || !canEditTranslation} className="py-2 px-5 rounded-md bg-brand-purple-base text-white font-semibold transition hover:bg-opacity-80 disabled:bg-opacity-50">
