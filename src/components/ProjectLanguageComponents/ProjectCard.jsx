...
const ProjectCard = ({ 
  ...
  const buttonBaseClass = `inline-flex items-center px-3 py-1.5 text-white rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-1`;
  
  const editButtonClass = disableHover 
    ? `${buttonBaseClass} ${editButtonColor} focus:ring-blue-500`
    : `${buttonBaseClass} ${editButtonColor} hover:bg-blue-700 focus:ring-blue-500 transition-colors`;
    
  const deleteButtonClass = disableHover
    ? `${buttonBaseClass} ${deleteButtonColor} focus:ring-red-500`
    : `${buttonBaseClass} ${deleteButtonColor} hover:bg-red-700 focus:ring-red-500 transition-colors`;

  // Card container classes
  const cardClasses = disableHover
    ? `${bgColor} border ${borderColor} rounded-lg p-6`
    : `${bgColor} border ${borderColor} rounded-lg p-6 transition-shadow hover:shadow-md`;
          ...
          <div className="flex flex-wrap gap-1.5">
            {project.languages && project.languages.length > 0 ? (
              project.languages.map((lang, idx) => {
                ...
                const badgeClasses = disableHover
